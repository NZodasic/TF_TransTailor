...
def load_data(batch_size):
    ...
    y_train = np.squeeze(y_train).astype('int64')  # Ensure int64 type
    y_test = np.squeeze(y_test).astype('int64')    # Ensure int64 type
...
def calculate_accuracy(model, dataset):
    ...
    for images, labels in dataset:
        ...
        # Explicitly cast both to the same type (tf.int64) before comparison
